<section id="inicio" class="relative overflow-hidden bg-gradient-to-r from-yellow-100 via-white to-red-50 pt-28 pb-16 sm:pt-32 sm:pb-20 lg:min-h-screen lg:flex lg:items-center">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-10 lg:gap-12 items-center z-10">
        <div class="order-2 lg:order-1 text-center lg:text-left">
            <p class="text-red-500 font-semibold uppercase tracking-[0.22em] sm:tracking-[0.3em] mb-3 text-xs sm:text-sm">
                Tradición y sabor
            </p>

            <h1 class="text-3xl sm:text-4xl md:text-5xl xl:text-7xl font-extrabold leading-tight text-gray-900">
                {{ $heroSlides[0]['title'] ?? 'El sabor que une a la familia' }}
            </h1>

            <p class="mt-5 sm:mt-6 text-base sm:text-lg md:text-xl text-gray-600 leading-relaxed max-w-xl mx-auto lg:mx-0">
            ...
            <div class="mt-8 sm:mt-10 grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-xl mx-auto lg:mx-0">
                <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-md p-4 text-center">
                ...
                <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-md p-4 text-center">
                    ...
                    <p class="text-sm text-gray-600">Momentos felices</p>
                </div>
            </div>
        </div>

        <div class="order-1 lg:order-2 relative">
            <div class="relative bg-white/70 backdrop-blur-md p-2 sm:p-3 rounded-[1.75rem] sm:rounded-[2rem] shadow-2xl">
                <div class="relative overflow-hidden rounded-[1.25rem] sm:rounded-[1.5rem]">
                    ...
                    <button
                        type="button"
                        id="heroPrev"
                        class="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white text-gray-900 w-9 h-9 sm:w-11 sm:h-11 rounded-full shadow-lg text-lg sm:text-xl font-bold transition flex items-center justify-center"
                    >
                        ‹
                    </button>
                    ...
                    <button
                        type="button"
                        id="heroNext"
                        class="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white text-gray-900 w-9 h-9 sm:w-11 sm:h-11 rounded-full shadow-lg text-lg sm:text-xl font-bold transition flex items-center justify-center"
                    >
                        ›
                    </button>
                </div>
                ...
                <div class="flex justify-center gap-2 sm:gap-3 mt-4 sm:mt-5">
                    @foreach($heroSlides as $index => $slide)
                        <button
                            type="button"
                            class="hero-dot w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full {{ $index === 0 ? 'bg-red-600' : 'bg-gray-300' }}"
    ...
    >
        <div class="relative max-w-5xl w-full">
            <button
                ...
                class="absolute -top-3 right-0 sm:-top-5 sm:-right-5 bg-white text-gray-900 rounded-full w-9 h-9 sm:w-10 sm:h-10 shadow-lg text-xl sm:text-2xl font-bold hover:bg-gray-100 transition"
            >
                ×
            </button>

            <div class="bg-white rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl">
                <img
                    id="heroModalImage"
                    src=""
                    alt=""
                ...
                <div class="p-4 sm:p-6">
                    <h3 id="heroModalTitle" class="text-xl sm:text-2xl font-extrabold text-red-600"></h3>
                </div>
            </div>
        </div>
    </div>
